...
- [ ] **Step 1:** Add `llama-cpp-2 = { version = "…", optional = true }`; `prompt = ["dep:llama-cpp-2"]`.
- [ ] **Step 2:** Implement model load + generation (best-effort code; the exact `llama-cpp-2` API may need small adjustments on first build — that's expected for a from-source binding).
- [ ] **Step 3 (OWNER):** `.\moonshine-build.bat` with `--features "moonshine prompt"`, place a Qwen2.5-**0.5B** Q4 GGUF (the lightweight default; try 1.5B too if you want to compare) in the models dir, run the app, select text, press the hotkey, and **judge the rewrite quality** (context preserved? reads well? latency acceptable? is 0.5B good enough, or is 1.5B worth the extra ~0.6 GB?).
