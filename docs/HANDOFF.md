...
### 2026-08-28 (later) — overlay not showing after sleep/resume (best-effort fix, needs on-device test)

Owner report: after the laptop sleeps and wakes, pressing the hotkey plays the start sound and turns on
the mic, **but the overlay pill never appears** — only a full restart (where AuraScribe auto-starts)
brings it back. Most likely cause: on Windows, **WebView2 suspends the hidden overlay webview across a
sleep/resume**, so `overlay::show` reveals a shown-but-blank transparent window (the pill never paints).

Fix (`overlay.rs` + `overlay/page.tsx`), low-risk, keeps the proven create-once + show/hide model:
`show()` now `unminimize()`s, positions, shows, re-asserts always-on-top, then **`window.eval(...)`s a
refresh hook** (`window.__auraOverlayRefresh`) the overlay page registers — running script forces the
webview to resume, and the hook re-reads authoritative status (`get_status`) and repaints the pill. Also
upgraded the show log to `info`. **Cannot be verified from the sandbox** (interactive sleep/resume on real
Windows) — the owner rebuilds and confirms; the `info` log now records every show. If it still recurs,
the next lever is off-screen-parking the window instead of hiding it (keeps WebView2 awake), or disabling
WebView2 suspension via `additionalBrowserArgs`.
